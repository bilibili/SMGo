...
//used registers:
//loadMasks: R8, R9, R10
//reverseBits: None general
//mul: None
//reduce:None
//load4X: data
//load1X: data

#define gHashBlocksPre(h,tag,data,count, tmp1, tmp2, tmp3) \
    VMOVDQU32   (h), VxH     \ // latency 7, CPI 0.5
	VMOVDQU32   (tag), VxTag  \ // higher lanes will be cleared automatically
	loadMasks(tmp1,tmp2,tmp3)    \
	MOVQ data, tmp1    \
	MOVQ count, tmp2    \
	MOVQ	            $GCM_POLY<>(SB), tmp3  \
	VBROADCASTI32X2     (tmp3), VzReduce     \  // latency 3, CPI 1
	reverseBits(VxH, VxAndMask, VxHigherMask, VxLowerMask, T1x, T2x)   \
	reverseBits(VxTag, VxAndMask, VxHigherMask, VxLowerMask, T1x, T2x) \
	VPSRLDQ     $8, VxH, VxHs      \
	VPXORD      VxH, VxHs, VxHs    \  //h^l in lower Hs

#define gHashBlocksLoopBy4Pre(h,tag,data,count, tmp1, tmp2, tmp3)  \
    MOVQ        $SHUFFLE_X_LANES<>(SB), tmp3  \
    VMOVDQU32   (tmp3), VzIdx   \
    MOVQ        $MERGE_H01<>(SB), tmp3  \
    VMOVDQU32   (tmp3), VyIdxH01  \
    MOVQ        $MERGE_H23<>(SB), tmp3  \
    VMOVDQU32   (tmp3), VzIdxH23  \
    MOVQ        $0b00001100, tmp3  \
    KMOVW       tmp3, MASK_Mov0_1   \
    MOVQ        $0b11110000, tmp3  \
    KMOVW       tmp3, MASK_Mov01_23  \
	mul(VxH, VxHs, VxH, VxLow, VxMid, VxHigh, T0x)   \
	reduce(U1x, VxReduce, VxLow, VxMid, VxHigh, T0x, T1x, T2x, T3x)  \
	mul(VxH, VxHs, U1x, VxLow, VxMid, VxHigh, T0x)    \
	reduce(U2x, VxReduce, VxLow, VxMid, VxHigh, T0x, T1x, T2x, T3x)  \
...
#define gHashBlocksLoopBy4(h,tag,data,count, tmp1, tmp2, tmp3) \
    load4X(data)         \
	VPXORD     VzTag, VzDat, VzDat  \
	mul(VzH4, VzH4s, VzDat, VzLow, VzMid, VzHigh, T0z)  \
	reduce(VzTag, VzReduce, VzLow, VzMid, VzHigh, T0z, T1z, T2z, T3z)  \
...
#define gHashBlocksLoopBy1(h,tag,data,count, tmp1, tmp2, tmp3) \
    load1X(data)   \
	VPXORD     VxTag, VxDat, VxDat  \
	mul(VxH, VxHs, VxDat, VxLow, VxMid, VxHigh, T0x)  \
	reduce(VxTag, VxReduce, VxLow, VxMid, VxHigh, T0x, T1x, T2x, T3x)   \
...
#define gHashBlocksBlocksEnd(h,tag,data,count, tmp1, tmp2, tmp3) \
    MOVQ tmp2, count         \
	MOVQ tmp1, data           \
	reverseBits(VxTag, VxAndMask, VxHigherMask, VxLowerMask, T1x, T2x)  \
	VMOVDQU32   VxTag, (tag)   \

#define gHashBlocksMacro(h,tag,data,count)  \
	gHashBlocksPre(h,tag,data,count, R10, R9, R8)   \
	CMPQ        count, $8          \ // this could be fine tuned based on benchmark results
    JL          loopBy1            \
	gHashBlocksLoopBy4Pre(h,tag,data,count, R10, R9, R8) \
	loopBy4:         \
	gHashBlocksLoopBy4(h,tag,data,count, R10, R9, R8) \
	CMPQ        count, $3    \
    JG          loopBy4      \
    CMPQ        count, $0    \
    JE          blocksEnd    \
	loopBy1:  \
	gHashBlocksLoopBy1(h,tag,data,count, R10, R9, R8) \
	CMPQ        count, $0   \
    JG          loopBy1     \
	blocksEnd:              \
	gHashBlocksBlocksEnd(h,tag,data,count, R10, R9, R8) \
...
//calculateJ0(h,counter,nonce,nonceLen,DX, nonceLen, tmp)
//used registers: R13,R15, BX,   R10, R9, R8
//
#define calculateJ0(h,tag,in,l,tmp, tmp1, tmp2,tmp3, tmp4, tmp5)  \
    MOVQ l, tmp1   \
    MOVQ l, tmp2   \
    ANDQ $15, tmp1 \
    SHRQ $4, tmp2  \
    CMPQ l, $16   \
    JL last       \
    gHashBlocksPre(h,tag,in,tmp2, tmp3, tmp4, tmp5)   \
	CMPQ        tmp2, $8          \ // this could be fine tuned based on benchmark results
    JL          loopBy1            \
	gHashBlocksLoopBy4Pre(h,tag,in,tmp2, tmp3, tmp4, tmp5) \
	loopBy4:         \
	gHashBlocksLoopBy4(h,tag,in,tmp2, tmp3, tmp4, tmp5) \
	CMPQ        tmp2, $3    \
    JG          loopBy4      \
    CMPQ        tmp2, $0    \
    JE          blocksEnd    \
	loopBy1:  \
	gHashBlocksLoopBy1(h,tag,in,tmp2, tmp3, tmp4, tmp5) \
	CMPQ        tmp2, $0   \
    JG          loopBy1     \
	blocksEnd:              \
	gHashBlocksBlocksEnd(h,tag,in,tmp2, tmp3, tmp4, tmp5) \
last:     \
    CMPQ tmp1, $0    \
    JE doneJ0        \
    MOVQ $0, (tmp)   \ //clear zero for tmp
    MOVQ $0, 8(tmp)  \
    MOVQ l, tmp3     \
    SUBQ tmp1, tmp3  \
    ADDQ tmp3, in    \
    MOVQ tmp1, tmp2   \
    copyAsm(tmp,in,tmp1,tmp3)  \
    SUBQ tmp2, tmp  \
    SUBQ tmp2, in   \
    MOVQ $1, tmp2   \
    gHashBlocksPre(h,tag,tmp,tmp2, tmp3, tmp4, tmp5)   \
    gHashBlocksLoopBy1(h,tag,tmp,tmp2, tmp3, tmp4, tmp5) \
    gHashBlocksBlocksEnd(h,tag,tmp,tmp2, tmp3, tmp4, tmp5) \
doneJ0:   \
    MOVQ $0, tmp3\
    SHLQ $3, l  \
    putUint64(tmp,tmp3)  \
...
//used registers: R13,R15, BX, R10, R9, R8   
#define calculateFirstCounterAsmMacro(nonce,nonceLen,counter,h,tmp, tmp1, tmp2,tmp3, tmp4, tmp5)  \  //DX, R11, R12, R10, R9
    CMPQ nonceLen, $12   \
    JE branch1           \
    calculateJ0(h,counter,nonce,nonceLen, tmp, tmp1, tmp2,tmp3, tmp4, tmp5)  \
    JMP doneCal     \
branch1:     \
    makeCounter(counter, nonce, tmp3)   \
doneCal:   \
    NOP  \
...
TEXT ·sealAsm(SB), NOSPLIT, $80-144    //80->88
    ...
    // DX, R11, R12, R10, R9, R14
    calculateFirstCounterAsmMacro(Nonce,NonceLen,J0,H,Tmp, DI, SI, AX, BX, CX)
